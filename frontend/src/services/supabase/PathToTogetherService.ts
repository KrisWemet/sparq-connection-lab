...
import { PathToTogetherJourney, PathToTogetherQuestion, UserPathToTogetherProgress } from './types'; // Renamed types
import { authService } from './AuthService';

/**
 * Service for handling Path To Together journey operations
 */
export class PathToTogetherService extends BaseService { // Renamed class
  /**
   * Get all available journeys
   */
  async getAllJourneys(): Promise<PathToTogetherJourney[]> { // Updated return type
    try {
   ...
   */
  async getJourneyById(journeyId: string): Promise<PathToTogetherJourney | null> { // Updated return type
    try {
      const { data, error } = await this.supabase
        .from('journeys')
        .select('*')
  ...
  async startJourney(journeyId: string): Promise<string | null> {
    try {
      ...
      await this.logActivity(user.id, 'journey_started', { 
        journeyId, 
        journeyTitle: (await this.getJourneyById(journeyId))?.title // Assuming title remains on the renamed type
      });
      
      return data.id;
    } catch (error: any) {
      return this.handleError(error, 'Failed to start Path To Together journey');
    }
  }
   ...
   */
  async getJourneyQuestions(journeyId: string): Promise<PathToTogetherQuestion[]> { // Updated return type
    try {
      const { data, error } = await this.supabase
  ...
  async submitJourneyResponse(journeyId: string, questionId: string, answer: string, reflection?: string): Promise<boolean> {
    try {
      ...
      return this.handleError(error, 'Failed to submit Path To Together response');
    }
  }
   ...
   */
  async getUserActiveJourneys(): Promise<UserPathToTogetherProgress[]> { // Updated return type
    try {
      const user = await authService.getCurrentUser();
      if (!user) return [];
      ...
      return data.map((uj: any) => ({
        ...
        // Assuming the nested 'journey' property will be renamed to 'pathToTogetherJourney' in the UserPathToTogetherProgress type
        pathToTogetherJourney: uj.journey ? {
          id: uj.journey.id,
      ...
      console.error('Error getting user active Path To Together journeys:', error.message);
      return [];
    }
  }
}
...
export const pathToTogetherService = new PathToTogetherService(); // Renamed instance