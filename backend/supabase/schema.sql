...
-- Definitions for daily_questions and daily_question_responses moved to migration files
-- (20250401150000_create_daily_questions.sql and 20250402100000_daily_question_responses.sql)
...
-- Drop potentially conflicting old policies just in case
...
-- Allow users to read their own profile OR the profile of the person who lists them as partner
CREATE POLICY "Allow read access for users to own and partner profiles"
...
-- Allow admins full access (ALL includes SELECT, INSERT, UPDATE, DELETE)
...
-- Shared events: Users can manage events created by them or their partner
ALTER TABLE public.shared_events ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view events created by them or their partner"
  ON public.shared_events FOR SELECT
  USING (
    auth.uid() = creator_id OR
    auth.uid() IN (SELECT partner_id FROM public.profiles WHERE id = creator_id) OR
    creator_id IN (SELECT partner_id FROM public.profiles WHERE id = auth.uid())
  );
...
CREATE POLICY "Users can update events created by them or their partner"
...
CREATE POLICY "Users can delete events created by them or their partner"
...
CREATE POLICY "Admins can manage all shared events"
  ON public.shared_events FOR ALL
  USING (public.is_admin(auth.uid()));

-- Communication prompts: Authenticated users can read all prompts
ALTER TABLE public.communication_prompts ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Authenticated users can read all communication prompts"
  ON public.communication_prompts FOR SELECT
  USING (auth.role() = 'authenticated');
-- Add policies for INSERT/UPDATE/DELETE later if needed (e.g., for admins)

-- Apply similar RLS policies to other tables...

-- Trigger function trigger_set_updated_at moved to migration file 20250401143000_create_trigger_function.sql
...
CREATE TRIGGER set_updated_at
BEFORE UPDATE ON public.communication_prompts
FOR EACH ROW EXECUTE FUNCTION public.trigger_set_updated_at();
...
-- Index for daily_question_responses moved to its migration file
...
CREATE INDEX idx_shared_events_creator_id ON public.shared_events(creator_id);
CREATE INDEX idx_communication_prompts_category ON public.communication_prompts(category);
