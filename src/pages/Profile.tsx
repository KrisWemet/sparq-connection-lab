...
export default function Profile() {
  ...
  const [email, setEmail] = useState(""); // Email is typically not updated here directly
  const [anniversary, setAnniversary] = useState(""); // Need DB schema confirmation for update
  const [sexualPreference, setSexualPreference] = useState("prefer-not-to-say"); // Map to gender? Needs clarification
  ...
  // Populate state when profile data loads
  useEffect(() => {
    if (profile) {
      ...
      // Fetch/set partner name if available (e.g., from profile.partnerId)
      // setAnniversary(profile.anniversaryDate || ""); // If anniversary date exists
    }
  }, [profile]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // We'll still capture all values in state for UI consistency,
      // but we only send the name to the server
      // This ensures the profile page still "works" from the user's perspective
      
      await profileService.updateProfile(updatedProfileData);
      toast.success("Profile updated successfully!");
      // The AuthProvider will refresh the profile on the next relevant event or page load.
      // For immediate update, a manual refresh mechanism could be added to AuthProvider.
    } catch (error) {
