...
export const QUESTIONS: Question[] = [
  ...
  // ── Q6: Emotional Speed ─────────────────────────────────────────────────────
  {
    ...
    captures: [],
    options: [
      { label: "It hits me fast and hard — I feel it right away", scoreDeltas: { dysregulation: 3, anxious: 2 }, traits: { space_preference: 'process_together' }, bridge: "Yeah — when you care, it's hard not to feel it. That makes sense." },
      { label: "I notice it, but I can usually breathe through it", scoreDeltas: { dysregulation: 1, secure: 1 }, traits: { space_preference: 'moves_between_both' }, bridge: "That's actually a real skill, even when it doesn't feel like one." },
      { label: "I tend to go quiet and not think about it much",   scoreDeltas: { avoidant: 2 }, traits: { space_preference: 'process_alone_first' }, bridge: "Stepping back to process — there's a kind of wisdom in that too." },
      WRITE_MY_OWN,
    ],
  },
  ...
  // ── Q11: Conflict Style ─────────────────────────────────────────────────────
  {
    ...
    options: [
      { label: "I bring it up — even when it's uncomfortable", sets: { field: 'conflictStyle', value: 'volatile'   }, traits: { repair_style: 'wants_direct_talk' },   bridge: "That directness — when it's timed right — is actually a gift to a relationship." },
      { label: "I need space to cool down before I can talk",  sets: { field: 'conflictStyle', value: 'avoidant'   }, scoreDeltas: { avoidant: 1 }, traits: { repair_style: 'needs_space_first' },    bridge: "Taking space before you speak is smarter than most people realise." },
      { label: "I try to make sure we both feel heard",        sets: { field: 'conflictStyle', value: 'validating' }, scoreDeltas: { secure: 1  }, traits: { repair_style: 'reaches_out_first' },    bridge: "That instinct to hold space for both of you — that's rare." },
      WRITE_MY_OWN,
    ],
  },
